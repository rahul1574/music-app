...
function Start() {
  ...
  return (
    ...
    <div id='start'style={{height:'500px',display:'flex',flexDirection:'column',justifyContent:'space-evenly',alignItems:'center'}}>
      ...
      <button id='wow' onClick={display} style={{height:'50px',width:'180px',borderRadius:'10px',fontSize:'17px',border:'none', display:'none'}}>7-AUG-2024 🎁</button>
       {btn && <ReactConfetti
       width={windowdimension.width}
       height={windowdimension.height}
       tweenDuration={1000}
       />}
       <h2 id='wish' style={{display:'none',color:'white'}}>LIFE IS BEAUTIFUL DUDE!😎</h2>
    </div>
    </>
  )
}
